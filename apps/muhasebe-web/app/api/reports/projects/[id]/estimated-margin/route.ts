...
export async function GET(request: Request, context: RouteContext) {
  try {
    ...
    await requirePermissions(
      user,
      routePermissions.reportRead("projectEstimatedMargin"),
      "Tahmini proje brut marj raporunu gorme yetkiniz yok",
    );

    const query = parseListQuery(request);
    const report = await getDbProjectEstimatedMarginReport(id, {
    ...
    return jsonBadRequest(error, "Tahmini proje brut marj raporu getirilemedi");
  }
}
