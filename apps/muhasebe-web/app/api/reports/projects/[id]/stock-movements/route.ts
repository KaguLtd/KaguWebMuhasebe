...
export async function GET(request: Request, context: RouteContext) {
  try {
    ...
    await requirePermissions(
      user,
      routePermissions.reportRead("projectStockMovements"),
      "Proje stok hareket raporunu gorme yetkiniz yok",
    );

    const query = parseListQuery(request);
    const report = await getDbProjectStockMovementReport(id, {
    ...
    return jsonBadRequest(error, "Proje stok hareketleri getirilemedi");
  }
}
