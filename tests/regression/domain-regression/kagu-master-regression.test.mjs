...
test("account and item code suggestions keep product prefixes", () => {
  assert.equal(
...
test("saveMaster falls back to the generated product code", () => {
...
test("project lookups carry account ownership for project filtering", () => {
...
test("document rule guard blocks account currency mismatch", () => {
  assert.throws(
    () =>
      guards.assertDraftDocumentRules("receipts", {
...
test("document rule guard blocks projects from another account", () => {
  assert.throws(
    () =>
      guards.assertDraftDocumentRules("invoices", {
...
test("different-currency transfers require a cross rate", () => {
  ...
  assert.throws(
    () =>
      guards.assertDraftDocumentRules("transfers", {
        amountMinor: 1000,
        currency: "TRY",
        docDate: "2026-05-04",
